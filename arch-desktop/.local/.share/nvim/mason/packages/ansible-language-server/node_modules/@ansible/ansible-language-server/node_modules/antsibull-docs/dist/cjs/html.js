...
function formatOptionLikeAntsibullDocs(part, url, what) {
    let link_start = '';
    let link_end = '';
    if (url) {
    ...
    return `<code class="${cls} literal notranslate">${strong_start}${link_start}${quoteHTML(text)}${link_end}${strong_end}</code>`;
}
const ANTSIBULL_DOCS_FORMATTER = {
    formatError: (part) => `<span class="error">ERROR while parsing: ${quoteHTML(part.message)}</span>`,
    formatBold: (part) => `<b>${quoteHTML(part.text)}</b>`,
...
function toHTML(paragraphs, opts) {
    var _a, _b;
    opts = opts !== null && opts !== void 0 ? opts : {};
    const style = opts.style || 'antsibull-docs';
    const mergedOpts = (0, opts_1.mergeOpts)(opts, style === 'antsibull-docs' ? ANTSIBULL_DOCS_FORMATTER : PLAIN_FORMATTER);
    const result = [];
    const parStart = (_a = opts.parStart) !== null && _a !== void 0 ? _a : '<p>';
    const parEnd = (_b = opts.parEnd) !== null && _b !== void 0 ? _b : '</p>';
    for (const paragraph of paragraphs) {
        result.push(parStart);
        (0, format_1.addToDestination)(result, paragraph, mergedOpts);
        result.push(parEnd);
    }
    return result.join('');
}
